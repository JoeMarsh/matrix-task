---
name: matrix-task
description: Complete complex software tickets with a requirements matrix, critical-invariant checks, phased implementation, clean-agent review, and evidence. Use when completeness and faithfulness to a substantial ticket matter more than speed or elegance.
---

# Matrix Task

You are working on a complex software ticket. Your highest priority is completeness and faithfulness to every requirement, not speed or elegance.

Work fully autonomously to the end of the ticket. Produce clear intermediate artifacts (matrix, gap list, self-checks, review notes) so a human can later inspect what happened. At the very end, list any remaining issues, limitations, or recommended follow-up work.

Only stop early if you hit a true hard blocker. Examples of hard blockers:

- The ticket depends on another ticket that is not yet done and whose absence makes correct implementation impossible.
...
- Critical information is missing that cannot be inferred or safely assumed from the ticket and codebase.
...
- A ticket expresses intent and an acceptance bar, not a rigid specification.
...
- When you deviate from the written approach, explicitly note the deviation, the reason, and how the acceptance bar is still met so it can be recorded on the ticket.

## Requirements Matrix

Do this first, before any code. Create a table with these columns:

| Requirement (quote or paraphrase the exact ticket text) | Category (functional / architectural invariant / conservation / test / non-goal) | Status in current codebase (exists / partial / missing / diverges) | Must remain true for all relevant actors/controllers? (yes/no + note) | Files / symbols that must change | How we will prove it (test name, assertion, or manual check) |
|---|---|---|---|---|---|
...
Extract and list every hard architectural or behavioural invariant from the ticket. Treat any violation of these invariants as a P0 defect, even if the happy path works.
...
The ticket is not done until:

- Every row in the Requirements Matrix is either fully satisfied and proven or explicitly marked out of scope with a ticket reference.
- All critical invariants still hold.
- The acceptance criteria in the original ticket are met with evidence (tests, digests, or clear reasoning).
...
- If you notice yourself implementing a convenient shortcut that the ticket forbids, stop and flag it.
...
- Any deviations from the original ticket approach.
- Remaining issues, limitations, or recommended follow-up tickets.
...
Start now with the Requirements Matrix for the ticket below.
